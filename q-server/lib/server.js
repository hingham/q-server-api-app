...
class Queue {
  ...
  connect(socket) {
    socket.on('subscribe', (event,cb) => {
      console.log(' in the connect method in server.js');
  ...
  static publish(message) {
    let {queue, event, payload, count, pings} = message;
    
    Queue.io.of(queue).to(event).emit('trigger', payload, count, pings);
    console.log('something was published', count, pings);
  }
